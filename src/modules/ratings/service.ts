...
export class RatingService {
  ...
  async requireTournamentAccess(userId: string, tournamentId: number): Promise<void> {
    ...
    // Вариант 1: Проверка по tournamentsIds в users (если там хранятся ID турниров)
    const isOrganizer = tournament.organizerId === userId;

    // Вариант 2: Проверка по email (временное решение)
    const isAdmin = user.email === Bun.env.ADMIN_EMAIL || user.isAdmin;

    // Вариант 3: Если есть отдельная таблица organizers или поле organizerId в tournaments
    // const isOrganizer = tournament.organizerId === userId;
  ...
  async loadTournamentMatches(tournamentId: number): Promise<TournamentMatch[]> {
    ...
    for (const match of matches) {
      tournamentMatches.push({
        redId: match.fighterRedId,
        blueId: match.fighterBlueId,
      ...
      tournamentMatches.push({
        redId: match.fighterBlueId,
        blueId: match.fighterRedId,
  ...
  */
  ...
  async processTournament(
    ...
    await db.transaction(async (tx) => {
        ...
        for (const match of matches) {
          const existingMatch = await tx.query.matches.findFirst({
            where: and(
              ...
              eq(schema.matches.fighterRedId, match.redId),
              eq(schema.matches.fighterBlueId, match.blueId),
          ...
          if (!existingMatch) {
            ...
            const [insertedMatch] = await tx.insert(schema.matches).values({
              tournamentId,
              fighterRedId: match.redId,
              fighterBlueId: match.blueId,
              nominationId,
      ...
      for (const result of results) {
        ...
        emitEvent(RATING_EVENTS.HISTORY_ADDED, { matchesPlayed: result.matchesPlayed, redId: result.redId })
      }
    ...
    return results.map(res=>({
      ...res,
      user: ratingRepository.getUserById(res.redId)
    }));
  }
   ...
   */
  async getLeaderboard(weaponId: number, nominationId: number, limit: number = 100) {
    ...
    if (cached) {
      return cached.topUsers.slice(0, limit);
    }
